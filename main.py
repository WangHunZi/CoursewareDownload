...
with (open(file_path, 'r', encoding='utf-8') as file):
    ...
    for link in reveal_links:
        ...
        if link.startswith("https://jyywiki.cn/../../static"):
            ...
            # 清理文件名中的特殊字符
        # print(sanitize_filename(src.split("/")[-1]))
        # script_filename = os.path.join(static_path, sanitize_filename(src.split("/")[-1]))
        # print(script_filename)
#         # 检查文件是否存在
#         if not os.path.exists(script_filename):
#             # 发送HTTP GET请求获取JavaScript脚本内容
#             response_script = requests.get(src)
            # 检查响应状态码
            # if response_script.status_code == 200:
            #     保存文件
                # with open(script_filename, "wb") as script_file:
                #     script_file.write(response_script.content)
                # print(f"已下载JS脚本文件: {script_file}")
            # else:
            #     print(f"无法下载JS脚本文件: {script_file}, 状态码: {response_script.status_code}")
    ...
    #     href = link.get('href')
        ...
        #     href = "https://jyywiki.cn" + href[2:]
        # 构建保存文件的路径
        # css_filename = os.path.join(slides_folder, sanitize_filename(href.split("/")[-1]))
        # print(css_filename)
        # if not os.path.exists(css_filename):
            # 发送HTTP GET请求获取CSS样式文件内容
            # response_css = requests.get(href)
            # 检查响应状态码
            # if response_css.status_code == 200:
                # 保存文件
                # with open(css_filename, "wb") as css_file:
                #     css_file.write(response_css.content)
                # print(f"已下载CSS样式文件: {css_filename}")
            # else:
            #     print(f"无法下载CSS样式文件: {css_filename}, 状态码: {response_css.status_code}")
